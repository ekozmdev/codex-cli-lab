...
- `git tag --sort=-v:refname` でタグ一覧を確認し、基準バージョンより新しい正式タグ（alpha/betaなどのプレリリースは除外）が存在するか確認する。
...
- `git show -s --format=%cs <最新タグ>` でリリース日（コミット日）を確認し、`yyyy-MM-dd` 形式で控える。
- `git diff <前タグ> <最新タグ>` で差分を取得する。

### レポート記述

- 差分から新機能や新しい設定項目など、重要な変更点を抜粋してまとめる（細かいバグ修正は記載不要）。有益な差分がなければ「特筆事項なし」と記載して終了してよい。
- `reports/yyyy-MM-dd-x-y-z.md` というMarkdownファイル（`yyyy-MM-dd` は取得したリリース日、`x-y-z` はバージョン）を新規作成し、レポートを保存する。既存レポートは上書きしない。公式タグに `rust-` などの接頭辞が付いていても、ファイル名からは除外する。
